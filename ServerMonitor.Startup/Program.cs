...
using ServerMonitor.Bot.HostedServices;
using Discord;
...
using ServerMonitor.Bot.Jobs;
using ServerMonitor.Bot.Services;
using ServerMonitor.Bot.Services.Games;
using ServerMonitor.Startup;
using ServerMonitor.Website.Components;
...
try
{
    ...
    builder.Services.AddHangfire((provider, configuration) => configuration.UseRedisStorage(provider.GetRequiredService<ConnectionMultiplexer>(), new RedisStorageOptions
        {
            Prefix = $"{Environment.GetEnvironmentVariable("PREFIX") ?? "servermonitor"}:hangfire:"
        }))
        .AddHangfireServer()
        .AddSingleton<StatusJob>()
        .AddSingleton<MonitorJob>();
    ...
    RecurringJob.AddOrUpdate<MonitorJob>("monitor", x => x.Check(), "0 */2 * * * *");
